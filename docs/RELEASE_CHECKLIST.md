...
3. `pyproject.toml` and `mechanism_ref.__version__` match the intended tag.
...
A public repository with an active license is open source, but a **formal project release** is recorded only when a version tag/release record is actually created and verified.
